...
l = listofzeros = [0.1] * 100
xls = pd.ExcelFile('UUCF Assignment Spreadsheet.xls')
...
for m in range(10,110):
    ...
    if(not(math.isnan(list(user_movieRating.loc[user_movieRating['User_Id'] == 3712][m])[0]))):
        ...
        for i,j in user_movieRating.iterrows():
            for k,l in Rel_Users.iterrows():                    
                if(not(math.isnan(user_movieRating[m][i]))):
                    count = count + 1;
                    if(count==1):
